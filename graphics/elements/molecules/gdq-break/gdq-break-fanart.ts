...
import {IAtomGridmaskImage} from '../../atoms/atom-gridmask-image/atom-gridmask-image';
...
window.addEventListener('load', () => {
	...
	class GdqBreakFanart extends InterruptMixin(Polymer.Element) {
		...
		_createEntranceAnim(tweet: Tweet) {
			...
			const $image = this.$.image as IAtomGridmaskImage;
			...
			tl.add((this.$.tweet as IGdqTweet)._createEntranceAnim(tweet), 'start+=0.1');
		...
		_createChangeAnim(tweet: Tweet) {
			...
			tl.call(() => {
				...
				exitTextTl.add((this.$.tweet as IGdqTweet)._createChangeAnim(tweet), 0);
				exitTextTl.call(() => {
		...
		_createExitAnim() {
			...
			tl.add((this.$.tweet as IGdqTweet)._createExitAnim(), 'exit');
			tl.add((this.$.image as IAtomGridmaskImage).exit(), 'exit+=0.1');
	...
	(window as any).GdqBreakFanart = GdqBreakFanart;
});
