...
window.addEventListener('load', () => {
	...
	class GdqBreakPrizes extends GDQBreakLoopMixin(Polymer.Element)<Prize> {
		ready() {
			super.ready();
		...
		show() {
			...
			const photoElem = this.$['photo-actual'] as IAtomGridmaskImage;
		...
		_resetState() {
			(this.$['photo-actual'] as IAtomGridmaskImage).exiting = false;
		}
	}

	// This assignment to window is unnecessary, but tsc complains that the class is unused without it.
	(window as any).GdqBreakPrizes = GdqBreakPrizes;
});
