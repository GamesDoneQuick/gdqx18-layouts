...
export interface IAtomTweeningNumber extends Polymer.Element {
	value: number;
	displayValueTransform: DisplayValueTransform;
	intervalLength: number;
	timePerValueInterval: number;
	maxDuration: number;
	ease: Ease;
	skipInitial: boolean;
	calcTweenDuration(deltaValue: number): number;
	tween(newValue: number, duration: number): TweenLite;
}
...
window.addEventListener('load', () => {
	...
	class AtomTweeningNumber extends Polymer.Element implements IAtomTweeningNumber {
		/**
	...
	(window as any).AtomTweeningNumber = AtomTweeningNumber;
});
